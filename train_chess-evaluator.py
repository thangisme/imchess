...
board_states = np.memmap(
    ...
    mode="r",
    dtype=np.uint8,
    shape=(total_number_of_positions, 8, 8, 12)
)
...
validation_raw_dataset = dataset_shuffled_once.take(
    number_of_validation_positions
)
training_raw_dataset   = dataset_shuffled_once.skip(
    number_of_validation_positions
)
...
def build_chess_evaluator_model(input_shape=(8, 8, 12)):
